...
def loss_fn(pred, y):
    return np.mean(np.log(1 + np.exp(-y * pred)))
def acc(pred, y):
...
def main():
    ...
    for epoch in range(N_EPOCH):
        ...
        for i in range(n_iter):
            ...
            y = train_y[iter_idx]
            pred = X @ weight
            grad = -np.mean((y * X.T) / (1 + np.exp(y * pred)), axis=1)
    ...
    plt.savefig('report/figures/lr/loss.pdf')
    plt.close()
    ...
    plt.legend()
    plt.tight_layout(pad=0.1)
    plt.savefig('report/figures/lr/acc.pdf')
    plt.close()
